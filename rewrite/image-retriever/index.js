...
async function fetchImage(url, wavelength, archive = false) {
    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            switch (wavelength) {
                case 'aia171':
                    (convertedSize > 645 && convertedSize < 1600) ? validSize = true : console.log(`${wavelength}: ${url} has an invalid file size of ${convertedSize}, skipping download.`);
                    break;
                case 'aia193':
                    (convertedSize > 475 && convertedSize < 1600) ? validSize = true : console.log(`${wavelength}: ${url} has an invalid file size of ${convertedSize}, skipping download.`);
                    break;
                case 'aia211':
                    ...
                    break;
                case 'aia304':
                    (convertedSize > 845 && convertedSize < 1600) ? validSize = true : console.log(`${wavelength}: ${url} has an invalid file size of ${convertedSize}, skipping download.`);
                    break;
                default:
